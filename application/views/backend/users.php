								...
								<table class="commonTable table table-striped table-bordered manage_user" cellspacing="0" width="100%">
									<thead>
										<tr>
											...
											<th>User Name</th>
											<th>Email</th>
											<?php if( $this->ts_functions->getsettings('portal','revenuemodel') == 'subscription' ) { echo '<th>Plan</th>'; } ?>
											<th>Status</th>
						    ...
						    ?>
									<tr>
										...
										<?php if( $this->ts_functions->getsettings('portal','revenuemodel') == 'subscription' ) {
										    $planDetails = $this->DatabaseModel->access_database('ts_plans','select','',array('plan_id'=>$solouser['user_plans']));
										echo !empty($planDetails) ? '<td>'.$planDetails[0]['plan_name'].'</td>' : '<td>FREE</td>'; } ?>
										<td>
										...
										<td>
                                        <a href="<?php echo $basepath?>backend/single_user/<?php echo $uid;?>"  class="delete" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a></td>
									</tr>
							<?php } } ?>
									<tbody>
								</table>
