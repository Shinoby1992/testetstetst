...
_msglog('e',"No plugins found in plugin directory '$plugindir'! <a href='".U."settings/plugin_force_remove/$plugindir/' style='color: white; text-decoration: underline;'>Click Here To</a> Remove This Plugin Entry.");
    ...
    public function css($path,$v=''){
        ...
        if(is_array($path)){
            ...
            foreach($path as $p){
                $gen .= '<link rel="stylesheet" type="text/css" href="'.APP_URL.'/sysfrm/plugins/'.$p.'.css'.$ver.'" />
        ';
            }
            ...
            $gen = '<link rel="stylesheet" type="text/css" href="'.APP_URL.'/sysfrm/plugins/'.$path.'.css'.$ver.'" />
        ';
        }
        return $gen;
    }
    ...
    public function js($path,$v=''){
        ...
        if(is_array($path)){
            foreach($path as $p){
                $gen .= '<script type="text/javascript" src="'.APP_URL.'/sysfrm/plugins/'.$p.'.js'.$ver.'"></script>
        ';
            }

        }
        else{
            $gen = '<script type="text/javascript" src="'.APP_URL.'/sysfrm/plugins/'.$path.'.js'.$ver.'"></script>
        ';
        }
        return $gen;
    }
